...
def preddict(max_accuracy, voting_clf_hard_accuracy, voting_clf_soft_accuracy, voting_clf_weighted):
    ...
    if max_accuracy == voting_clf_hard_accuracy:
        y_pred = voting_clf_hard.predict(X)
    elif max_accuracy == voting_clf_soft_accuracy:
        y_pred = voting_clf_soft.predict(X)
    else:
        y_pred = voting_clf_weighted.predict(X)
    output_text.delete(1.0, tk.END)

    for i in range(6):
        if y_pred[i] == 0:
            output_text.insert(tk.END, f"Dominated \n")
        elif y_pred[i] == 1:
            output_text.insert(tk.END, f"Pure \n")
        elif y_pred[i]== 2:
            output_text.insert(tk.END, f"Forest \n")
        elif y_pred[i]== 3:
            output_text.insert(tk.END, f"Water \n")
        elif y_pred[i]== 4:
            output_text.insert(tk.END, f"Mixed Bamboo \n")
        elif y_pred[i]== 5:
            output_text.insert(tk.END, f"Fallowland \n")
        else:
            output_text.insert(tk.END, f"Unknown class label: {y_pred[i]}")
    

def classify():
    global max_accuracy, voting_clf_hard_accuracy, voting_clf_soft_accuracy, voting_clf_weighted, voting_clf_hard, voting_clf_soft
    file_path = path_entry.get()
    df = pd.read_csv(file_path)
    df['latitude'] = df['.geo'].apply(lambda x: json.loads(x)['coordinates'][1])
    df['longitude'] = df['.geo'].apply(lambda x: json.loads(x)['coordinates'][0])
    ...
    max_accuracy = max(voting_clf_hard_accuracy, voting_clf_soft_accuracy, voting_clf_weighted_accuracy)
    ...
    #output_text.insert(tk.END, f"CART accuracy: {cart_accuracy*100:.2f}\n")
...
query_label = tk.Label(root, text="Select CSV file to predict class labels:")
query_label.pack()
...
classify_label = tk.Label(root, text="Click button to predict:")
classify_label.pack()
classify_button = tk.Button(root, text="Predict", command=lambda: preddict(max_accuracy, voting_clf_hard_accuracy, voting_clf_soft_accuracy, voting_clf_weighted))
classify_button.pack()
